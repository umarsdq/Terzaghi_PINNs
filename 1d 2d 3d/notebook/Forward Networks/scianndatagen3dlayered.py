...
class DataGeneratorXYZT:
    ...
    # Arguments:
      ...
      targets: list and type of targets you wish to impose on PINNs. 
          ('domain', 'ic', 'bc-left', 'bc-right', 'bc-front', 'bc-back', 'bc-bottom', 'bc-top', 'interface-1-2', 'interface-2-3', 'all')
    ...
    # Examples: 
      >> dg = DataGeneratorXYZT([0., 1.], [0., 1.], [0., 1.], [0., 1.], 
                               ["domain", "ic", "bc-left", "bc-right", "bc-front", "bc-back", "bc-bottom", "bc-top", "interface-1-2", "interface-2-3"], 
                               10000)
    ...
    def generate_data(self):
        ...
        # bc points 
        num_sample_per_edge = int(num_sample/8)
        # left bc points 
        ...
        y_bc_top = np.random.uniform(self.Ydomain[0], self.Ydomain[1], num_sample_per_edge)
        z_bc_top = np.full(num_sample_per_edge, self.Zdomain[1])
        t_bc_top = self.generate_uniform_T_samples(num_sample_per_edge)
        ids_bc_top = np.arange(x_bc_top.shape[0]) + counter
        counter += ids_bc_top.size

        # interface 1-2 bc points 
        z_interface_1_2 = self.Zdomain[1] * 2 / 3
        x_bc_interface_1_2 = np.random.uniform(self.Xdomain[0], self.Xdomain[1], num_sample_per_edge)
        y_bc_interface_1_2 = np.random.uniform(self.Ydomain[0], self.Ydomain[1], num_sample_per_edge)
        z_bc_interface_1_2 = np.full(num_sample_per_edge, z_interface_1_2)
        t_bc_interface_1_2 = self.generate_uniform_T_samples(num_sample_per_edge)
        ids_bc_interface_1_2 = np.arange(x_bc_interface_1_2.shape[0]) + counter
        counter += ids_bc_interface_1_2.size

        # interface 2-3 bc points 
        z_interface_2_3 = self.Zdomain[1] * 1 / 3
        x_bc_interface_2_3 = np.random.uniform(self.Xdomain[0], self.Xdomain[1], num_sample_per_edge)
        y_bc_interface_2_3 = np.random.uniform(self.Ydomain[0], self.Ydomain[1], num_sample_per_edge)
        z_bc_interface_2_3 = np.full(num_sample_per_edge, z_interface_2_3)
        t_bc_interface_2_3 = self.generate_uniform_T_samples(num_sample_per_edge)
        ids_bc_interface_2_3 = np.arange(x_bc_interface_2_3.shape[0]) + counter
        counter += ids_bc_interface_2_3.size

        ids_bc = np.concatenate([ids_bc_left, ids_bc_right, ids_bc_front, ids_bc_back, ids_bc_bottom, ids_bc_top, ids_bc_interface_1_2, ids_bc_interface_2_3])
        ...
        ids = {
            ...
            'bc-top': ids_bc_top,
            'interface-1-2': ids_bc_interface_1_2,
            'interface-2-3': ids_bc_interface_2_3,
            'ic': ids_ic,
            'bc': ids_bc, 
            'all': ids_all
        }
        ...
        input_data = [
            np.concatenate([x_dom, x_ic, x_bc_left, x_bc_right, x_bc_front, x_bc_back, x_bc_bottom, x_bc_top, x_bc_interface_1_2, x_bc_interface_2_3]).reshape(-1,1),
            np.concatenate([y_dom, y_ic, y_bc_left, y_bc_right, y_bc_front, y_bc_back, y_bc_bottom, y_bc_top, y_bc_interface_1_2, y_bc_interface_2_3]).reshape(-1,1),
            np.concatenate([z_dom, z_ic, z_bc_left, z_bc_right, z_bc_front, z_bc_back, z_bc_bottom, z_bc_top, z_bc_interface_1_2, z_bc_interface_2_3]).reshape(-1,1),
            np.concatenate([t_dom, t_ic, t_bc_left, t_bc_right, t_bc_front, t_bc_back, t_bc_bottom, t_bc_top, t_bc_interface_1_2, t_bc_interface_2_3]).reshape(-1,1),
        ]
